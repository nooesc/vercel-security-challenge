...
const TEST_ID = "SBX-057-WILDCARD-EMPTY-ISOLATION" as const;
...
export interface Sbx057PendingTransaction {
  operation: "acquire" | "release";
  mode: "normal" | "cleanup-only";
}

export interface Sbx057RemovalSettlement {
  canonicalRemoved: boolean;
  transactionOperation?: "acquire" | "release";
}

export type Sbx057LiveLockMutation =
...
type MutationHook = (mutation: Sbx057LiveLockMutation) => void | Promise<void>;
...
function parseLiveLock(value: unknown): LiveLockMetadata {
  if (value === null || typeof value !== "object" || Array.isArray(value) || !exactKeys(value, [
    ...
    throw new Error("SBX-057 live-lock metadata fields were not exact");
  }
  ...
  if (candidate.schemaVersion !== 1 || candidate.testId !== TEST_ID || candidate.kind !== "live-lock" ||
    ...
    throw new Error("SBX-057 live-lock metadata was invalid");
  }
...
function parseTransaction(value: unknown): TransactionMetadata {
  if (value === null || typeof value !== "object" || Array.isArray(value) || !exactKeys(value, [
    ...
    throw new Error("SBX-057 lock-transaction metadata fields were not exact");
  }
  const candidate = value as Record<string, unknown>;
  ...
  if (candidate.schemaVersion !== 1 || candidate.testId !== TEST_ID ||
    ...
    throw new Error("SBX-057 lock-transaction metadata was invalid");
  }
...
function parseTransactionFinalizer(value: unknown): TransactionFinalizerMetadata {
  if (value === null || typeof value !== "object" || Array.isArray(value) || !exactKeys(value, [
    ...
    throw new Error("SBX-057 transaction-finalizer metadata fields were not exact");
  }
  const candidate = value as Record<string, unknown>;
  ...
  if (candidate.schemaVersion !== 1 || candidate.testId !== TEST_ID ||
    ...
    throw new Error("SBX-057 transaction-finalizer metadata was invalid");
  }
...
function currentUserId(): bigint {
  if (typeof process.getuid !== "function") {
    throw new Error("SBX-057 live locking requires a POSIX process user ID");
  }
...
function noFollow(): number {
  if (typeof constants.O_NOFOLLOW !== "number" || constants.O_NOFOLLOW === 0) {
    throw new Error("SBX-057 live locking requires O_NOFOLLOW support");
  }
...
async function assertSecureDirectory(path: string): Promise<void> {
  ...
  if (!metadata.isDirectory() || metadata.uid !== currentUserId() ||
    ...
    throw new Error("SBX-057 live-lock directory must be current-user-owned with exact mode 0700");
  }
}
...
async function readBoundedJson(handle: FileHandle): Promise<unknown> {
  ...
  try {
    ...
    if (total === 0 || total > MAXIMUM_METADATA_BYTES) {
      throw new Error("SBX-057 live-lock metadata exceeded its fixed byte bound");
    }
    try {
      return JSON.parse(retained.subarray(0, total).toString("utf8"));
    } catch {
      throw new Error("SBX-057 live-lock metadata was not valid JSON");
    }
  } finally {
    retained.fill(0);
  }
...
async function claimAndRemoveOwned<T>(
  ...
  equal: (left: Readonly<T>, right: Readonly<T>) => boolean,
  hook?: MutationHook,
  mutation?: Sbx057LiveLockMutation,
): Promise<void> {
...
async function createTransaction(
  ...
  throw new Error("SBX-057 transaction raced existing finalization state");
}
...
async function removalClaimPaths(path: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    if (!/^[1-9][0-9]*-[A-Za-z0-9_-]{43}$/u.test(entry.slice(prefix.length))) {
      throw new Error("SBX-057 removal-claim name was malformed");
    }
  }
...
async function transactionFinalizerSidecars(path: string): Promise<{
  ...
  for (const entry of elections) {
    if (!LEASE.test(entry.slice(`${base}.election-`.length))) {
      throw new Error("SBX-057 transaction-finalizer election name was malformed");
    }
  }
  for (const entry of replacements) {
    if (!/^[1-9][0-9]*-[A-Za-z0-9_-]{43}$/u.test(entry.slice(`${base}.next-`.length))) {
      throw new Error("SBX-057 transaction-finalizer replacement name was malformed");
    }
  }
  return {
...
async function settleTransactionFinalizerSidecars(
  ...
  if (sidecars.elections.length > 1) {
    throw new Error("SBX-057 transaction-finalizer recovery found multiple elections");
  }
  ...
  if (sidecars.elections.length === 1) {
    ...
    const election = await openSecureMetadata(
      electionPath,
      "SBX-057 transaction-finalizer election",
      parseTransactionFinalizer,
    );
    let fixed = await optionalTransactionFinalizer(path);
    try {
      if (!exactFinalizerTarget(election.metadata, target)) {
        throw new Error("SBX-057 transaction-finalizer election targeted another transaction");
      }
      if (fixed !== undefined && sameIdentity(fixed, election) &&
        ...
        if (fixed.metadata.lease === targetLease) {
          throw new Error("SBX-057 transaction-finalizer election reused its target generation");
        }
        const installedOwner = ownerState(election.metadata.pid);
        if (installedOwner === "live") {
          throw new Error("SBX-057 installed transaction-finalizer owner is live");
        }
        if (installedOwner !== "dead") {
          throw new Error("SBX-057 installed transaction-finalizer liveness was uncertain");
        }
        await removeExactClaim(
          electionPath,
          election,
        ...
        if (electionOwner === "live") {
          throw new Error("SBX-057 transaction-finalizer takeover owner is live");
        }
        if (electionOwner !== "dead") {
          throw new Error("SBX-057 transaction-finalizer takeover liveness was uncertain");
        }
        ...
        if (installed === undefined || !sameIdentity(installed, election) ||
          ...
          throw new Error("SBX-057 transaction-finalizer recovery installed another generation");
        }
        await installed.handle.close();
      } else {
        throw new Error("SBX-057 transaction-finalizer election had unknown fixed-path provenance");
      }
    } finally {
      await fixed?.handle.close().catch(() => undefined);
  ...
  try {
    for (const replacementPath of replacements) {
      const replacement = await openSecureMetadata(
        replacementPath,
        "SBX-057 transaction-finalizer replacement",
        parseTransactionFinalizer,
      );
      try {
        if (!exactFinalizerTarget(replacement.metadata, target)) {
          throw new Error("SBX-057 transaction-finalizer replacement targeted another transaction");
        }
        ...
        if (state === "live") {
          throw new Error("SBX-057 transaction-finalizer replacement owner is live");
        }
        if (state !== "dead") {
          throw new Error("SBX-057 transaction-finalizer replacement liveness was uncertain");
        }
        await removeExactClaim(
          replacementPath,
          replacement,
...
async function acquireTransactionFinalizer(
  ...
  try {
    if (!exactFinalizerTarget(existing.metadata, target)) {
      throw new Error("SBX-057 transaction finalizer targeted another transaction");
    }
    const state = ownerState(existing.metadata.pid);
    if (state === "live") throw new Error("SBX-057 transaction finalizer owner is live");
    if (state !== "dead") throw new Error("SBX-057 transaction finalizer liveness was uncertain");

    const metadata = transactionFinalizerMetadata(target);
    if (metadata.lease === existing.metadata.lease) {
      throw new Error("SBX-057 transaction-finalizer replacement repeated a lease");
    }
    ...
    try {
      if (current === undefined || !sameIdentity(current, existing) ||
        ...
        throw new Error("SBX-057 transaction finalizer changed during elected takeover");
      }
    } finally {
      await current?.handle.close();
    }
    ...
    try {
      if (installed === undefined || !sameIdentity(installed, replacement) ||
        ...
        throw new Error("SBX-057 transaction-finalizer replacement lost fixed-path ownership");
      }
    } finally {
      await installed?.handle.close();
    }
...
async function requireOwnedTransactionFinalizer(
  ...
  try {
    if (current === undefined || !sameIdentity(current, expected) ||
      ...
      throw new Error("SBX-057 transaction-finalizer ownership changed");
    }
  } finally {
    await current?.handle.close();
  }
...
async function optionalLiveLock(path: string): Promise<OpenedSecureFile<LiveLockMetadata> | undefined> {
  return optionalLiveLockAt(path, "SBX-057 live lock");
}
...
async function recoverInterruptedAcquireAtPath(
  ...
  if (!isAbsolute(path)) throw new Error("SBX-057 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 acquire recovery requires a canonical UUIDv4");
  ...
  if (transaction === undefined) {
    ...
    try {
      ...
      if (ownerState(dangling.metadata.pid) !== "dead") {
        throw new Error("SBX-057 dangling acquire-finalizer owner was not proven dead");
      }
      ...
      if (reappeared !== undefined) {
        ...
        throw new Error("SBX-057 acquire transaction reappeared while settling its finalizer");
      }
  ...
  try {
    if (transaction.metadata.runId !== runId) {
      throw new Error("SBX-057 refused a mismatched interrupted acquire transaction");
    }
    if (ownerState(transaction.metadata.pid) !== "dead") {
      throw new Error("SBX-057 refused to recover an acquire whose owner was not proven dead");
    }
    ...
    try {
      if (currentTransaction === undefined || !sameIdentity(currentTransaction, transaction) ||
        ...
        throw new Error("SBX-057 interrupted acquire transaction changed after finalizer election");
      }
    } finally {
    ...
    replacement = await optionalLiveLockAt(
      replacementPath,
      "SBX-057 interrupted acquire replacement lock",
    );
    stale = await optionalLiveLockAt(stalePath, "SBX-057 interrupted acquire stale lock");

    if (replacement !== undefined && !exactAcquireTarget(replacement.metadata, transaction.metadata)) {
      throw new Error("SBX-057 interrupted acquire replacement was not its exact target lease");
    }
    if (stale !== undefined && !exactDeadAcquireSource(stale.metadata, transaction.metadata)) {
      throw new Error("SBX-057 interrupted acquire stale lock was not its exact dead source run");
    }
    ...
    if (canonical !== undefined && !canonicalIsTarget && !canonicalIsSource) {
      throw new Error("SBX-057 interrupted acquire canonical lock had unknown provenance");
    }
    if (canonicalIsTarget && canonical !== undefined && replacement !== undefined &&
      ...
      throw new Error("SBX-057 interrupted acquire target paths did not share one exact inode");
    }

    if (transaction.metadata.mode === "normal") {
      if (stale !== undefined || canonicalIsSource) {
        throw new Error("SBX-057 normal interrupted acquire unexpectedly contained a stale source");
      }
      if (canonicalIsTarget && canonical !== undefined) {
        await removeExactClaim(path, canonical, "SBX-057 live lock", parseLiveLock, sameLiveLock);
      ...
      if (stale !== undefined) {
        if (!sameIdentity(canonical, stale) || !sameLiveLock(canonical.metadata, stale.metadata)) {
          throw new Error("SBX-057 interrupted acquire duplicated a different stale source");
        }
        await removeExactClaim(
          stalePath,
          stale,
          "SBX-057 interrupted acquire stale lock",
          parseLiveLock,
          sameLiveLock,
        );
      ...
      try {
        ...
        if (errorCode(error) === "EEXIST") {
          throw new Error("SBX-057 interrupted acquire could not restore its exact stale source");
        }
        throw error;
      }
      await hook?.("recover-source-linked");
      const restored = await openSecureMetadata(path, "SBX-057 restored stale live lock", parseLiveLock);
      try {
        if (!sameIdentity(restored, stale) || !sameLiveLock(restored.metadata, stale.metadata)) {
          throw new Error("SBX-057 interrupted acquire restored a different stale source");
        }
      } finally {
      ...
      throw new Error("SBX-057 cleanup takeover lost both its exact source and target");
    }
...
async function settleRemovalClaimsAtPath(
  path: string,
  runId: string,
): Promise<Sbx057RemovalSettlement> {
  if (!isAbsolute(path)) throw new Error("SBX-057 removal-claim recovery path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 removal-claim recovery requires a canonical UUIDv4");
  await assertSecureDirectory(path);
...
async function inspectPendingTransactionAtPath(
  path: string,
  runId: string,
): Promise<Sbx057PendingTransaction | undefined> {
  if (!isAbsolute(path)) throw new Error("SBX-057 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 transaction inspection requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const transaction = await optionalTransaction(path);
  if (transaction === undefined) {
    ...
    try {
      if (finalizer.metadata.runId !== runId) {
        throw new Error("SBX-057 refused a mismatched pending transaction finalizer");
      }
      return {
  ...
  try {
    if (transaction.metadata.runId !== runId) {
      throw new Error("SBX-057 refused a mismatched pending transaction");
    }
...
async function rollbackOrphanedNormalLockAtPath(
  ...
  if (!isAbsolute(path)) throw new Error("SBX-057 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 orphan rollback requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  ...
  if (pendingTransaction !== undefined) {
    ...
    throw new Error("SBX-057 orphan rollback refused a pending transaction");
  }
  ...
  try {
    if (canonical.metadata.runId !== runId || canonical.metadata.mode !== "normal") {
      throw new Error("SBX-057 orphan rollback refused a foreign or non-normal lock");
    }
    const state = ownerState(canonical.metadata.pid);
    if (state === "live") throw new Error("SBX-057 orphan rollback refused a live lock owner");
    if (state !== "dead") throw new Error("SBX-057 orphan lock owner liveness was uncertain");
    ...
    if (sidecars.length !== 0) {
      throw new Error("SBX-057 orphan rollback refused acquisition sidecars");
    }
    ...
    try {
      if (current === undefined || !sameIdentity(current, canonical) ||
        ...
        throw new Error("SBX-057 orphan lock changed after release serialization");
      }
    } finally {
      await current?.handle.close();
    }
...
async function resumeInterruptedReleaseAtPath(
  ...
  if (!isAbsolute(path)) throw new Error("SBX-057 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 release recovery requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const settledRemoval = await settleRemovalClaimsAtPath(path, runId);
  const transaction = await optionalTransaction(path);
  if (transaction === undefined && settledRemoval.transactionOperation === "release") {
    ...
    if (canonical !== undefined) {
      await canonical.handle.close();
      throw new Error("SBX-057 finalized release claim retained a canonical lock");
    }
    return true;
  }
  if (transaction === undefined) {
    ...
    try {
      if (dangling.metadata.runId !== runId || dangling.metadata.targetOperation !== "release") {
        throw new Error("SBX-057 dangling transaction finalizer did not identify this release");
      }
      if (ownerState(dangling.metadata.pid) !== "dead") {
        throw new Error("SBX-057 dangling transaction-finalizer owner was not proven dead");
      }
      const target = targetFromFinalizer(dangling.metadata);
      await dangling.handle.close();
      finalizer = await acquireTransactionFinalizer(path, target, hook);
      ...
      if (reappeared !== undefined) {
        await reappeared.handle.close();
        throw new Error("SBX-057 transaction reappeared while settling its finalizer");
      }
      const canonical = await optionalLiveLock(path);
      if (canonical !== undefined) {
        try {
          if (canonical.metadata.runId !== runId || canonical.metadata.pid !== target.targetPid ||
            ...
            throw new Error("SBX-057 dangling release finalizer did not match its canonical lock");
          }
          await removeExactClaim(path, canonical, "SBX-057 live lock", parseLiveLock, sameLiveLock);
        } finally {
          await canonical.handle.close().catch(() => undefined);
  ...
  try {
    if (transaction.metadata.operation !== "release" || transaction.metadata.runId !== runId ||
      ...
      throw new Error("SBX-057 refused a mismatched or non-release transaction");
    }
    if (ownerState(transaction.metadata.pid) !== "dead") {
      throw new Error("SBX-057 refused to resume a release whose owner was not proven dead");
    }
    finalizer = await acquireTransactionFinalizer(path, transaction.metadata, hook);
    await requireOwnedTransactionFinalizer(path, finalizer);
    const currentTransaction = await optionalTransaction(path);
    try {
      if (currentTransaction === undefined || !sameIdentity(currentTransaction, transaction) ||
          !sameTransaction(currentTransaction.metadata, transaction.metadata)) {
        throw new Error("SBX-057 release transaction changed after finalizer election");
      }
    } finally {
      await currentTransaction?.handle.close();
    }
    ...
    if (canonical !== undefined &&
      ...
      throw new Error("SBX-057 interrupted release canonical lock did not match its transaction");
    }
    if (canonical !== undefined) {
      await removeExactClaim(path, canonical, "SBX-057 live lock", parseLiveLock, sameLiveLock);
      await canonical.handle.close();
...
async function rollbackInterruptedReleaseAtPath(
  ...
  if (!isAbsolute(path)) throw new Error("SBX-057 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 release rollback requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const settledRemoval = await settleRemovalClaimsAtPath(path, runId);
  const transaction = await optionalTransaction(path);
  if (transaction === undefined) {
    ...
    try {
      if (dangling.metadata.runId !== runId || dangling.metadata.targetOperation !== "release" ||
        ...
        throw new Error("SBX-057 dangling rollback finalizer was not an exact dead release");
      }
      const target = targetFromFinalizer(dangling.metadata);
      const canonical = await optionalLiveLock(path);
      try {
        if (canonical === undefined || canonical.metadata.runId !== runId ||
          ...
          throw new Error("SBX-057 dangling rollback finalizer lacked its exact canonical lock");
        }
      } finally {
        await canonical?.handle.close();
      }
      ...
      if (reappeared !== undefined) {
        await reappeared.handle.close();
        throw new Error("SBX-057 release transaction reappeared during rollback finalizer settlement");
      }
      await releaseTransactionFinalizer(path, finalizer);
      finalizer = undefined;
      return true;
  ...
  try {
    ...
    if (ownerState(transaction.metadata.pid) !== "dead") {
      throw new Error("SBX-057 refused to roll back a release whose owner was not proven dead");
    }
    finalizer = await acquireTransactionFinalizer(path, transaction.metadata, hook);
    await requireOwnedTransactionFinalizer(path, finalizer);
    canonical = await optionalLiveLock(path);
    if (canonical === undefined || canonical.metadata.runId !== runId ||
        ...
        canonical.metadata.lease !== transaction.metadata.targetLease ||
        canonical.metadata.mode !== transaction.metadata.mode) {
      throw new Error("SBX-057 interrupted release lacked its exact canonical lock");
    }
    const currentTransaction = await optionalTransaction(path);
    try {
      if (currentTransaction === undefined || !sameIdentity(currentTransaction, transaction) ||
          !sameTransaction(currentTransaction.metadata, transaction.metadata)) {
        throw new Error("SBX-057 release transaction changed after rollback election");
      }
    } finally {
      await currentTransaction?.handle.close();
    }
...
async function requireCanonicalAbsent(path: string): Promise<void> {
  ...
  throw new Error("SBX-057 fixed live-lock path survived or was replaced during release");
}

class HeldLiveLock implements Sbx057HeldLock {
  ...
  async closeRetainingState(): Promise<void> {
    ...
    if (failures.length > 0) {
      throw new AggregateError(failures.map((failure) => failure.reason),
        "SBX-057 failed to close retained lock descriptors");
    }
  ...
  async releaseAfter(preCommit: () => Promise<void>): Promise<void> {
    ...
    if (this.closedRetainingState) {
      throw new Error("SBX-057 cannot release a lock whose descriptors were closed while retaining state");
    }
...
async function acquireAtPath(
  ...
  recovery: boolean,
  hook?: MutationHook,
): Promise<Sbx057HeldLock> {
  if (!isAbsolute(path)) throw new Error("SBX-057 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-057 live lock requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  ...
  try {
    ...
    if (!recovery && existing !== undefined) {
      throw new Error("SBX-057 refused a normal run because the live lock already exists");
    }
    if (recovery) {
      if (existing === undefined || existing.metadata.runId !== runId) {
        throw new Error("SBX-057 cleanup-only mode refused a missing or mismatched live lock");
      }
      const state = ownerState(existing.metadata.pid);
      if (state === "live") throw new Error("SBX-057 cleanup-only mode refused a live lock owner");
      if (state !== "dead") throw new Error("SBX-057 live-lock owner liveness was uncertain");
    }
    ...
    if (existing !== undefined) {
      ...
      const claimed = await openSecureMetadata(staleClaimPath, "SBX-057 stale live lock", parseLiveLock);
      try {
        if (!sameIdentity(claimed, existing) || !sameLiveLock(claimed.metadata, existing.metadata)) {
          throw new Error("SBX-057 stale live lock changed during atomic claim");
        }
      } finally {
    ...
    const canonical = await openSecureMetadata(path, "SBX-057 live lock", parseLiveLock);
    try {
      if (!sameIdentity(canonical, replacement) || !sameLiveLock(canonical.metadata, metadata)) {
        throw new Error("SBX-057 replacement live lock lost inode or lease ownership");
      }
    } finally {
      await canonical.handle.close();
    }
...
export async function acquireSbx057LiveLock(
  path: string,
  runId: string,
  recovery: boolean,
): Promise<Sbx057HeldLock> {
...
export async function acquireSbx057LiveLockAtPathForTest(
  path: string,
  runId: string,
  recovery: boolean,
  hook?: MutationHook,
...
export async function resumeSbx057InterruptedRelease(
  path: string,
  runId: string,
): Promise<boolean> {
...
export async function resumeSbx057InterruptedReleaseAtPathForTest(
  path: string,
  runId: string,
  hook?: MutationHook,
): Promise<boolean> {
...
export async function rollbackSbx057InterruptedRelease(
  path: string,
  runId: string,
): Promise<boolean> {
...
export async function rollbackSbx057InterruptedReleaseAtPathForTest(
  path: string,
  runId: string,
  hook?: MutationHook,
): Promise<boolean> {
...
export async function recoverSbx057InterruptedAcquire(
  path: string,
  runId: string,
): Promise<boolean> {
...
export async function recoverSbx057InterruptedAcquireAtPathForTest(
  path: string,
  runId: string,
  hook?: MutationHook,
): Promise<boolean> {
...
export async function inspectSbx057PendingTransaction(
  path: string,
  runId: string,
): Promise<Sbx057PendingTransaction | undefined> {
...
export async function settleSbx057RemovalClaims(
  path: string,
  runId: string,
): Promise<Sbx057RemovalSettlement> {
...
export async function rollbackSbx057OrphanedNormalLock(
  path: string,
  runId: string,
): Promise<boolean> {
...
export async function rollbackSbx057OrphanedNormalLockAtPathForTest(
  path: string,
  runId: string,
  hook?: MutationHook,
): Promise<boolean> {
