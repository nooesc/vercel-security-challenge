...
const TEST_ID = "SBX-053-GIT-CREDENTIAL-RETENTION" as const;
...
export type Sbx053LiveLockMutation = "acquire-canonical-installed" |
  "release-transaction-created" |
  "release-precommit-complete" | "release-canonical-removed";
type MutationHook = (mutation: Sbx053LiveLockMutation) => void | Promise<void>;
...
function parseLiveLock(value: unknown): LiveLockMetadata {
  if (value === null || typeof value !== "object" || Array.isArray(value) || !exactKeys(value, [
    ...
    throw new Error("SBX-053 live-lock metadata fields were not exact");
  }
  ...
  if (candidate.schemaVersion !== 1 || candidate.testId !== TEST_ID || candidate.kind !== "live-lock" ||
    ...
    throw new Error("SBX-053 live-lock metadata was invalid");
  }
...
function parseTransaction(value: unknown): TransactionMetadata {
  if (value === null || typeof value !== "object" || Array.isArray(value) || !exactKeys(value, [
    ...
    throw new Error("SBX-053 lock-transaction metadata fields were not exact");
  }
  const candidate = value as Record<string, unknown>;
  ...
  if (candidate.schemaVersion !== 1 || candidate.testId !== TEST_ID ||
    ...
    throw new Error("SBX-053 lock-transaction metadata was invalid");
  }
...
function currentUserId(): bigint {
  if (typeof process.getuid !== "function") {
    throw new Error("SBX-053 live locking requires a POSIX process user ID");
  }
...
function noFollow(): number {
  if (typeof constants.O_NOFOLLOW !== "number" || constants.O_NOFOLLOW === 0) {
    throw new Error("SBX-053 live locking requires O_NOFOLLOW support");
  }
...
async function assertSecureDirectory(path: string): Promise<void> {
  ...
  if (!metadata.isDirectory() || metadata.uid !== currentUserId() ||
    ...
    throw new Error("SBX-053 live-lock directory must be current-user-owned with exact mode 0700");
  }
}
...
async function readBoundedJson(handle: FileHandle): Promise<unknown> {
  ...
  try {
    ...
    if (total === 0 || total > MAXIMUM_METADATA_BYTES) {
      throw new Error("SBX-053 live-lock metadata exceeded its fixed byte bound");
    }
    try {
      return JSON.parse(retained.subarray(0, total).toString("utf8"));
    } catch {
      throw new Error("SBX-053 live-lock metadata was not valid JSON");
    }
  } finally {
    retained.fill(0);
  }
...
function lockMetadata(runId: string, mode: LockMode, lease: string): LiveLockMetadata {
  if (!LEASE.test(lease)) throw new Error("SBX-053 live-lock acquisition lease was invalid");
  return {
    schemaVersion: 1,
    testId: TEST_ID,
    kind: "live-lock",
...
async function optionalLiveLock(path: string): Promise<OpenedSecureFile<LiveLockMetadata> | undefined> {
  return optionalLiveLockAt(path, "SBX-053 live lock");
}
...
async function resumeInterruptedReleaseAtPath(path: string, runId: string): Promise<boolean> {
  if (!isAbsolute(path)) throw new Error("SBX-053 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-053 release recovery requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const transaction = await optionalTransaction(path);
  if (transaction === undefined) return false;
  if (transaction.metadata.operation !== "release") {
  ...
  try {
    if (transaction.metadata.runId !== runId ||
      ...
      throw new Error("SBX-053 refused a mismatched release transaction");
    }
    if (ownerState(transaction.metadata.pid) !== "dead") {
      throw new Error("SBX-053 refused to resume a release whose owner was not proven dead");
    }
    ...
    if (canonical !== undefined &&
      ...
      throw new Error("SBX-053 interrupted release canonical lock did not match its transaction");
    }
    if (canonical !== undefined) {
      await removeExactClaim(path, canonical, "SBX-053 live lock", parseLiveLock, sameLiveLock);
...
async function rollbackInterruptedReleaseAtPath(path: string, runId: string): Promise<boolean> {
  if (!isAbsolute(path)) throw new Error("SBX-053 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-053 release rollback requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const transaction = await optionalTransaction(path);
  if (transaction === undefined) return false;
  if (transaction.metadata.operation !== "release") {
  ...
  try {
    ...
    if (ownerState(transaction.metadata.pid) !== "dead") {
      throw new Error("SBX-053 refused to roll back a release whose owner was not proven dead");
    }
    canonical = await optionalLiveLock(path);
    if (canonical === undefined || canonical.metadata.runId !== runId ||
        ...
        canonical.metadata.lease !== transaction.metadata.targetLease ||
        canonical.metadata.mode !== transaction.metadata.mode) {
      throw new Error("SBX-053 interrupted release lacked its exact canonical lock");
    }
    await removeExactClaim(
      transactionPath(path),
      transaction,
...
async function resumeInterruptedAcquireAtPath(path: string, runId: string): Promise<boolean> {
  if (!isAbsolute(path)) throw new Error("SBX-053 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-053 acquire recovery requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const transaction = await optionalTransaction(path);
  if (transaction === undefined || transaction.metadata.operation !== "acquire") {
    await transaction?.handle.close();
    return false;
  }
  let canonical: OpenedSecureFile<LiveLockMetadata> | undefined;
  ...
  try {
    if (transaction.metadata.runId !== runId) {
      throw new Error("SBX-053 refused a mismatched interrupted acquire transaction");
    }
    if (ownerState(transaction.metadata.pid) !== "dead") {
      throw new Error("SBX-053 refused to resume an acquire whose owner was not proven dead");
    }
    canonical = await optionalLiveLock(path);
    if (canonical === undefined || canonical.metadata.runId !== runId ||
        canonical.metadata.pid !== transaction.metadata.pid ||
        canonical.metadata.lease !== transaction.metadata.lease ||
        canonical.metadata.mode !== transaction.metadata.mode) {
      throw new Error("SBX-053 interrupted acquire lacked its exact installed canonical lock");
    }

    const replacementPath =
      `${path}.next-${transaction.metadata.pid}-${transaction.metadata.lease}`;
    replacement = await optionalLiveLockAt(
      replacementPath,
      "SBX-053 interrupted acquire replacement lock",
    );
    if (replacement !== undefined) {
      if (!sameIdentity(replacement, canonical) ||
          !sameLiveLock(replacement.metadata, canonical.metadata)) {
        throw new Error("SBX-053 interrupted acquire replacement did not match its canonical lock");
      }
      await removeExactClaim(
        replacementPath,
        replacement,
    ...
    const stalePath =
      `${path}.stale-${transaction.metadata.pid}-${transaction.metadata.lease}`;
    stale = await optionalLiveLockAt(stalePath, "SBX-053 interrupted acquire stale lock");
    if (stale !== undefined) {
      if (stale.metadata.runId !== runId || ownerState(stale.metadata.pid) !== "dead") {
        throw new Error("SBX-053 interrupted acquire stale lock was not exact and dead");
      }
      await removeExactClaim(
        stalePath,
        stale,
        "SBX-053 interrupted acquire stale lock",
        parseLiveLock,
        sameLiveLock,
      );
...
async function rollbackInterruptedAcquireAtPath(path: string, runId: string): Promise<boolean> {
  if (!isAbsolute(path)) throw new Error("SBX-053 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-053 acquire rollback requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  const transactionResumed = await resumeInterruptedAcquireAtPath(path, runId);
  const canonical = await optionalLiveLock(path);
  if (canonical === undefined) {
    if (transactionResumed) {
      throw new Error("SBX-053 interrupted acquire canonical lock disappeared during rollback");
    }
    return false;
  }
  try {
    if (canonical.metadata.runId !== runId) {
      throw new Error("SBX-053 refused a mismatched interrupted acquire lock");
    }
    if (ownerState(canonical.metadata.pid) !== "dead") {
      throw new Error("SBX-053 refused to roll back an acquire whose owner was not proven dead");
    }
  } finally {
    await canonical.handle.close();
  }

  const reclaimed = await acquireAtPath(path, runId, true);
...
async function requireCanonicalAbsent(path: string): Promise<void> {
  ...
  throw new Error("SBX-053 fixed live-lock path survived or was replaced during release");
}

class HeldLiveLock implements Sbx053HeldLock {
  ...
  async closeRetainingState(): Promise<void> {
    ...
    if (failures.length > 0) {
      throw new AggregateError(failures.map((failure) => failure.reason),
        "SBX-053 failed to close retained lock descriptors");
    }
  ...
  async releaseAfter(preCommit: () => Promise<void>): Promise<void> {
    ...
    if (this.closedRetainingState) {
      throw new Error("SBX-053 cannot release a lock whose descriptors were closed while retaining state");
    }
...
async function acquireAtPath(
  ...
  if (!isAbsolute(path)) throw new Error("SBX-053 live-lock path must be absolute");
  if (!UUID_V4.test(runId)) throw new Error("SBX-053 live lock requires a canonical UUIDv4");
  await assertSecureDirectory(path);
  await resumeInterruptedAcquireAtPath(path, runId);
  ...
  try {
    ...
    if (!recovery && existing !== undefined) {
      throw new Error("SBX-053 refused a normal run because the live lock already exists");
    }
    if (recovery) {
      if (existing === undefined || existing.metadata.runId !== runId) {
        throw new Error("SBX-053 cleanup-only mode refused a missing or mismatched live lock");
      }
      const state = ownerState(existing.metadata.pid);
      if (state === "live") throw new Error("SBX-053 cleanup-only mode refused a live lock owner");
      if (state !== "dead") throw new Error("SBX-053 live-lock owner liveness was uncertain");
    }
    ...
    if (existing !== undefined) {
      ...
      const claimed = await openSecureMetadata(staleClaimPath, "SBX-053 stale live lock", parseLiveLock);
      try {
        if (!sameIdentity(claimed, existing) || !sameLiveLock(claimed.metadata, existing.metadata)) {
          throw new Error("SBX-053 stale live lock changed during atomic claim");
        }
      } finally {
    ...
    const canonical = await openSecureMetadata(path, "SBX-053 live lock", parseLiveLock);
    try {
      if (!sameIdentity(canonical, replacement) || !sameLiveLock(canonical.metadata, metadata)) {
        throw new Error("SBX-053 replacement live lock lost inode or lease ownership");
      }
    } finally {
      await canonical.handle.close();
    }
...
export async function acquireSbx053LiveLock(
  path: string,
  runId: string,
  recovery: boolean,
): Promise<Sbx053HeldLock> {
...
export async function acquireSbx053LiveLockAtPathForTest(
  path: string,
  runId: string,
  recovery: boolean,
  hook?: MutationHook,
...
export async function resumeSbx053InterruptedRelease(
  path: string,
  runId: string,
): Promise<boolean> {
...
export async function rollbackSbx053InterruptedRelease(
  path: string,
  runId: string,
): Promise<boolean> {
...
/** Rolls back only an exact dead acquisition when no recovery journal was committed. */
export async function rollbackSbx053InterruptedAcquire(
  path: string,
  runId: string,
): Promise<boolean> {
  return rollbackInterruptedAcquireAtPath(path, runId);
}
